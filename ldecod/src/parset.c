...
void reset_format_info(seq_parameter_set_rbsp_t *sps, VideoParameters *p_Vid, FrameFormat *source, FrameFormat *output)
{
  ...
  if (p_Vid->first_sps == TRUE) {
   ...
   /*   if(p_Inp->idetails==1){
      fprintf(p_Inp->fp,"YUV Format: %d Source: %dx%d Video: (%dx%d)\n",p_Vid->yuv_format, source->width[0], source->height[0], p_Vid->width, p_Vid->height);
	     if(p_Inp->ichroma==1){      
		fprintf(p_Inp->fpb,"YUV Format : %d Source: %dx%d Video: (%dx%d)\n", p_Vid->yuv_format, source->width[0], source->height[0], p_Vid->width, p_Vid->height);
      		fprintf(p_Inp->fpr,"YUV Format : %d Source: %dx%d Video: (%dx%d)\n",p_Vid->yuv_format, source->width[0], source->height[0], p_Vid->width, p_Vid->height);
	} 
