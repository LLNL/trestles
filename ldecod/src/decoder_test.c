...
static void Configure(InputParameters *p_Inp, int ac, char *av[])
{
  ...
  //mvs_patterns = fopen(p_Inp->smbfile,"w");
  mb_out = fopen(p_Inp->mbfile,"w");
  mb_info = fopen(p_Inp->mbinfo,"w");
  mv_out = fopen(p_Inp->mvfile,"w");
  fpf = fopen(p_Inp->coeffile,"w");
  fpb = fopen(p_Inp->coeffile_b,"w");
  fpr = fopen(p_Inp->coeffile_r,"w");
...
int main(int argc, char **argv)
{
  ...
  if(p_Inp->ichroma!=1){
  fprintf(p_Inp->fpr,"Red Chroma coefficients not requested, change decoder.cfg parameter to 1 if needed.");
  fprintf(p_Inp->fpb,"Blue Chroma coefficients not requested, change decoder.cfg parameter to 1 if needed."); 	
  } 
  if(p_Inp->mv_flag!=1){
  fprintf(p_Inp->mv_out,"Motion Vectors not requested, change decoder.cfg parameter to 1 if needed");
 }
  fclose(p_Inp->fpr);
  fclose(p_Inp->fpb);
  fclose(p_Inp->mv_out);
  fclose(p_Inp->mb_out);
  fclose(p_Inp->mb_info);
