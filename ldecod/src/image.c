...
int decode_one_frame(DecoderParams *pDecoder)
{
  ...
  snr->previous_frame = -1;   // bsb 4/6/2016
  ...
  *dec_picture=NULL;
  ...
  if ((structure==FRAME)||structure==BOTTOM_FIELD)
  {
    ...
    sprintf(yuvFormat,"%s", yuv_types[chroma_format_idc]);
      ...
      if(p_Vid->frame_no==0){snr->reset=snr->frame_ctr;}  // bsb 3/21/16
      snr->disp_order=(snr->reset+p_Vid->frame_no);
   //   fprintf(p_Inp->fp,"global frame number: %d \n", snr->disp_order);
    if (p_Inp->silent == FALSE)
    {
      SNRParameters   *snr = p_Vid->snr;
...
void decode_one_slice(Slice *currSlice)
{
  ...
  SNRParameters *snr = p_Vid->snr;
  StorablePicture *dec_picture_b = currSlice->dec_picture;
  int *modifications = currSlice->long_term_pic_idx;
  ...
  VideoParameters *mb_Vid = currMB->p_Vid;
 if(currMB->mb_type==0){
	    counter_s = counter_s + 1;
	}
    else if(currMB->mb_type>8){
   ...
   if(p_Vid->frame_no==0){
    snr->reset  = snr->sframe_ctr;
   }
   fprintf(p_Inp->mb_info,"%d,%d,%d,%d,%d  \n",currMB->mb_type,currMB->delta_quant,currMB->block_x,currMB->block_y,p_Vid->frame_no+snr->reset);
  ...
  fprintf(p_Inp->mb_out,"%d, %d, %d, %d, %s \n",counter_i,counter_s,counter_p,snr->reset+p_Vid->frame_no,p_Vid->cslice_type);
    ++(snr->sframe_ctr);
  //reset_ec_flags(p_Vid);
}
