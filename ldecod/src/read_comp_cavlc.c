...
************************************************************************
* \brief
*    Extract coefficients and write them to text file(s)
************************************************************************
*/


static void extract_coeffs(Macroblock *currMB, int ***coefficients){
 ...
 calculate_frame_no(p_Vid, currSlice->dec_picture);

	if(p_Vid->frame_no > snr->prev_max){
	snr->prev_max = p_Vid->frame_no;
	}

 snr->disp_order = snr->reset + p_Vid->frame_no;
 
 	if(p_Vid ->frame_no == 0){
 	   if(snr->reset==0 && snr->prev_max!=0){
        snr->disp_order = snr->reset + snr->prev_max + 1;
        }
        else{
        snr->disp_order = snr->reset + snr->prev_max; 
        }
        snr->prev_max = 0;
 	}
 

  if(currSlice->slice_type == I_SLICE && currSlice->idr_flag){ // IDR picture
        strcpy(p_Vid->cslice_type,"IDR");
  
	//snr->disp_order = snr->frame_ctr;
	}  // bsb bug fix 4/6/2016: need to reset for IDR frames
      else if(currSlice->slice_type == I_SLICE) // I picture
        strcpy(p_Vid->cslice_type," I ");
      else if(currSlice->slice_type == P_SLICE) // P pictures
        strcpy(p_Vid->cslice_type," P ");
      else if(currSlice->slice_type == SP_SLICE) // SP pictures
        strcpy(p_Vid->cslice_type,"SP ");
      else if (currSlice->slice_type == SI_SLICE)
        strcpy(p_Vid->cslice_type,"SI ");
      else if(currSlice->nal_reference_idc) // stored B pictures
...
if(p_Inp->ichroma == 1){
	...
	if((snr->disp_order != snr->previous_frame) && (p_Inp->idetails==1)){
		if(p_Inp->iiframes==1){
			if(strncmp(" I ",p_Vid->cslice_type,3)==0 || strncmp("IDR",p_Vid->cslice_type,3)==0){
		//	fprintf(p_Inp->fpb,"Frame number: %d Type: %s \n", snr->disp_order,p_Vid->cslice_type);
		//	fprintf(p_Inp->fpr,"Frame number: %d Type: %s \n", snr->disp_order,p_Vid->cslice_type);
		//	fprintf(p_Inp->fp,"Frame number: %d Type: %s \n", snr->disp_order,p_Vid->cslice_type);
		}
		}
		else{//fprintf(p_Inp->fpb,"Frame number: %d Type: %s \n", snr->disp_order,p_Vid->cslice_type);
		//	fprintf(p_Inp->fpr,"Frame number: %d Type: %s \n", snr->disp_order,p_Vid->cslice_type);
		//	fprintf(p_Inp->fp,"Frame number: %d Type: %s \n", snr->disp_order,p_Vid->cslice_type);
		}
	}
   ...
   for(int i=0;i<16;i++){
 	for(int j=0;j<16;j++){ 
 	  if(coefficients[0][j][i]!=0){
		    if(p_Inp->iiframes==1){
			if(strncmp(" I ",p_Vid->cslice_type,3)==0 || strncmp("IDR",p_Vid->cslice_type,3)==0){
				if(p_Inp->idetails==0){fprintf(p_Inp->fp,"%d \n", coefficients[0][j][i]);}
				else{//fprintf(p_Inp->fp,"%d,%d,%d  \n",currMB->pix_x+i, currMB->pix_y+j, coefficients[0][j][i]);
					   fprintf(p_Inp->fp,"%d,%d,%d,%d,%d,%d \n", snr->disp_order,currMB->block_x,currMB->block_y,i,j, coefficients[0][j][i]);
				}
				...
				if(p_Inp->idetails==0){fprintf(p_Inp->fp,"%d \n", coefficients[0][j][i]);}
				else{fprintf(p_Inp->fp,"%d,%d,%d,%d,%d,%d  \n",snr->disp_order,currMB->block_x,currMB->block_y,i,j, coefficients[0][j][i]);}
			}	
	  }
 		if(coefficients[1][j][i]!=0 ){ 
 			if(p_Inp->iiframes==1){
				if(strncmp(" I ",p_Vid->cslice_type,3)==0 || strncmp("IDR",p_Vid->cslice_type,3)==0){
					if(p_Inp->idetails==0){fprintf(p_Inp->fpb,"%d \n", coefficients[1][j][i]);}
					else{fprintf(p_Inp->fpb,"%d,%d,%d,%d,%d,%d  \n",snr->disp_order,currMB->block_x,currMB->block_y,i,j, coefficients[1][j][i]);}
				}
			}
			else{ 
				if(p_Inp->idetails==0){fprintf(p_Inp->fpb,"%d \n", coefficients[1][j][i]);}
		...
		if(coefficients[2][j][i]!=0){ 
			if(p_Inp->iiframes==1){
				if(strncmp(" I ",p_Vid->cslice_type,3)==0 || strncmp("IDR",p_Vid->cslice_type,3)==0){
					if(p_Inp->idetails==0){fprintf(p_Inp->fpr,"%d \n", coefficients[2][j][i]);}
					else{fprintf(p_Inp->fpr,"%d,%d,%d,%d,%d,%d  \n",snr->disp_order,currMB->block_x,currMB->block_y,i,j, coefficients[2][j][i]);}
				}
			}
			else{ 
				if(p_Inp->idetails==0){fprintf(p_Inp->fpr,"%d \n", coefficients[2][j][i]);}
...
snr->previous_frame = snr->disp_order;
...
{ 
 ...
 //  currSlice->cof_quant = currSlice->cof;
  ...
  for (block_y = 0; block_y < 4; block_y += 2) /* all modes */
  {
    ...
    for (block_x = 0; block_x < 4; block_x += 2)
    {
      ...
      if (cbp & (1 << b8))  // test if the block contains any coefficients
      {
		...
		currSlice->cof_quant[pl][j + j0][i + i0]= levarr[k];


	/*	if(p_Inp->idctquant==0){
                	currSlice->cof[pl][j + j0][i + i0]= rshift_rnd_sf((levarr[k] * InvLevelScale4x4[j0][i0])<<qp_per, 4);
		}
		else{
			currSlice->cof[pl][j + j0][i + i0]= levarr[k];
		} */
...
static void read_comp_coeff_8x8_CAVLC (Macroblock *currMB, ColorPlane pl, int (*InvLevelScale8x8)[8], int qp_per, int cbp, byte **nzcoeff)
{
  ...
  for (block_y = 0; block_y < 4; block_y += 2) /* all modes */
  {
	...
	/*	if(p_Inp->idctquant==0){
		...
		currSlice->mb_rres[pl][block_y4 +j0][block_x4 +i0] = levarr[k];
		}*/
              }
...
if(p_Inp->idctquant==1){
extract_coeffs(currMB,currSlice->cof_quant);
}
else{
extract_coeffs(currMB,currSlice->cof);
}
}
...
//currSlice->cof_quant = currSlice->cof;
  ...
  else
  {
	...
	currSlice->cof[PLANE_U + uv][(j<<2) + j0][(i<<2) + i0] = rshift_rnd_sf((levarr[k] * InvLevelScale4x4[j0][i0])<<qp_per_uv[uv], 4);
 	currSlice->cof_quant[PLANE_U + uv][(j<<2) + j0][(i<<2) + i0] = levarr[k];
 /*	if(p_Inp->idctquant==0){
              currSlice->cof[PLANE_U + uv][(j<<2) + j0][(i<<2) + i0] = rshift_rnd_sf((levarr[k] * InvLevelScale4x4[j0][i0])<<qp_per_uv[uv], 4);
		}
		else{
  ...
  if(cbp>15)
  {
    for (ll=0;ll<3;ll+=2)
    {
 	...
 	currSlice->cof[PLANE_U + uv][0][0] = (((temp[0] * InvLevelScale4x4[0][0])<<qp_per_uv[uv])>>5);
       ...
       currSlice->cof_quant[PLANE_U + uv][0][0] = temp[0];

       currSlice->cof_quant[PLANE_U + uv][0][4] = temp[1];

        currSlice->cof_quant[PLANE_U + uv][4][0] = temp[2];

        currSlice->cof_quant[PLANE_U + uv][4][4] = temp[3];
	...
	/*if(p_Inp->idctquant==0){
       ...
       currSlice->cof[PLANE_U + uv][0][0] = temp[0];
        currSlice->cof[PLANE_U + uv][0][4] = temp[1];
        currSlice->cof[PLANE_U + uv][4][0] = temp[2];
        currSlice->cof[PLANE_U + uv][4][4] = temp[3];
	}*/
      }          
    }     
  }
